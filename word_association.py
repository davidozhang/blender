class WordAssociation(object):
    def __init__(self, lines=[]):
        ...
        for line in lines:
            self.create_association(line.split())

    '''
    Given a list of words, adds all the words to existing associations keyed by 1 or more of the words. Otherwise,
    creates a new association and adds all the words to that association.
    '''
    def associate(self, words):
        ...
        if len(indices) == 0:
            indices = self.create_association(words)
    ...
    '''
    def create_association(self, words):
